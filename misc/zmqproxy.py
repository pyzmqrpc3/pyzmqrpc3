...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Proxies message either from SUB->REQ, SUB->PUB, REP->PUB or REP->REQ.')
    parser.add_argument(
        '--sub',
        nargs='+',
        required=False,
    ...
    parser.add_argument(
        '--username_incoming',
        required=False,
        help='In case a username is needed for the incoming connection (sub, rep)')
    parser.add_argument(
        '--password_incoming',
        required=False,
        help='In case a password is needed for the incoming connection (sub, rep)')
    parser.add_argument(
        '--username_outgoing',
        required=False,
        help='In case a username is needed for the outgoing connection (req, pub)')
    parser.add_argument(
        '--password_outgoing',
        required=False,
        help='In case a password is needed for the outgoing connection (req, pub)')
    ...
    if args.sub is not None and args.rep is not None:
        logger.error(
            "Fatal error: Proxy cannot listen to both SUB and REP at the same time")
    elif args.pub is not None and args.req is not None:
        logger.error(
            "Fatal error: Proxy cannot send messages over both PUB and REQ sockets at the same time")
        ...
        if args.sub is not None and args.req is not None:
            server = ZmqProxySub2Req(
                ...
                zmq_req_connect_addresses=args.req,
                username_sub=args.username_incoming,
            ...
            server = ZmqProxyRep2Pub(
                ...
                zmq_pub_bind_address=args.pub,
                username_rep=args.username_incoming,
