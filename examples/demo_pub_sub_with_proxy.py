...
def main(args: Optional[Tuple[str]] = None) -> int:
    ...
    proxy_server = ZmqProxySub2PubThread(
        zmq_pub_bind_address='tcp://*:30001',
        zmq_sub_connect_addresses=['tcp://localhost:30000'],
    )
    proxy_server.start()

    print('starting server ...')
    server = ZmqRpcServerThread(
        zmq_sub_connect_addresses=['tcp://localhost:30001'],    # Must be a list
    )
